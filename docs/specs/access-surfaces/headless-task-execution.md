...
**R8.** On a valid model the shim SHALL echo it in the submit acknowledgement (`{status: accepted, task_id, harness, model}`; key present only when requested) and forward it to the headless argv as a discrete `--model <id>` pair: `opencode run [--session <id>] --model <id> [--agent …] --auto <prompt>` and `claude -p [--resume <id>] --model <id> [--agent …] --dangerously-skip-permissions <prompt>`; for pi the model is forwarded as the provider/model pair of `pi -p`. When the flag is omitted, the payload has no `model` field and the argv is unchanged (the harness resolves its own default model).
...
**I7.** The `model` field is present in the payload/ack/state if and only if a model was requested; absence always means "harness default".
...
- Shim implementation: [image/app/main.py](../../../image/app/main.py) (`_handle_task_action` validation and `continue` echo, post-ready continuation resolution (`_resolve_continue_session`, outcome persisted via `_persist_continue_outcome`), `_build_headless_argv`, `effective_model`, orphan reconciliation, `SCH_TASK_TIMEOUT_S`).
