...
**R1.** The CLI SHALL expose `sch task <workspace> [--harness <opencode|claude|pi>] [--model <id>] [--continue] "<prompt>" [--timeout <s>]` which sends the `task` action to the shim via `InvokeAgentRuntime` and returns immediately with a `task_id`, without waiting for the prompt to complete. The client MUST NOT keep a persistent connection for the duration of the task; completion is observable later with `sch status`. The payload SHALL include the `harness` field so the shim builds the correct argv.
...
**R8.** On a valid model the shim SHALL echo it in the submit acknowledgement (`{status: accepted, task_id, harness, model}`; key present only when requested) and forward it to the headless argv as a discrete `--model <id>` pair: `opencode run [--session <id>] --model <id> [--variant <v>] [--agent …] --auto <prompt>` and `claude -p [--resume <id>] --model <id> [--agent …] --dangerously-skip-permissions <prompt>`; for pi the model is forwarded as the provider/model pair of `pi -p`. When the flag is omitted, the payload has no `model` field and the argv is unchanged (the harness resolves its own default model), except for opencode `--continue` (R8a), which forwards the resumed session's own stored model and reasoning-effort variant.

**R8a.** On opencode `--continue` without an explicit `--model`, the shim SHALL forward the resumed session's own stored model (`provider/model` from the `opencode.db` session row) as `--model <id>` and its stored reasoning-effort variant as `--variant <v>` (opencode's provider-specific effort selector, e.g. `high`; the `"default"` sentinel and any malformed value are omitted). Rationale: the headless argv switches the agent to `remote-auto`, and opencode resolves a model-less prompt to the new agent's configured model and default effort — the TUI-selected model and effort would be lost and the session row clobbered. An explicit `--model` always wins (the stored variant is then dropped: it belongs to the previous model). A stored provider that cannot serve at runtime (neither in the seeded config, nor backed by a staged provider key, nor `amazon-bedrock` via the execution role) still resolves to the runtime default model with no variant, and any read or validation failure degrades to the pre-feature behavior (no `--model`/`--variant` pair; the harness default applies). There is deliberately no `--variant` CLI flag on `sch task` or `sch run`: an explicit effort is unsupported, the resumed session's effort is the only way to select one.
...
**R23.** `sch task <workspace> --handoff [--handoff-session <id>] [--sanitize]` SHALL export the local OpenCode session (most recent session of the current directory by default, `--handoff-session` bypasses the resolution, `--sanitize` delegates redaction to `opencode export --sanitize`), transfer it through the bundle channel, import it via the `session-import` action, and only then submit the `task` action with `continue=true` (the flag is implied by `--handoff`; an explicit `--continue` is accepted and redundant). `--handoff-session` and `--sanitize` without `--handoff` SHALL die with a usage error before any mutation.
...
- Opencode `--continue` without `--model` → the resumed session keeps the model and reasoning effort selected in the TUI (forwarded as `--model`/`--variant`); an explicit `--model` overrides the model and drops the stored effort. There is no `--variant` flag: an explicit effort cannot be requested.
...
**I7.** The `model` field is present in the payload/ack/state if and only if a model was requested; absence means "harness default", except on opencode `--continue`, where the resumed session's stored model/variant is forwarded to the argv (R8a) without being recorded as a requested model.
